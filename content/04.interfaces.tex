...
\begin{figure}[!htbp]
	...
	\caption[Donjon, performance audiovisuelle utilisant des bornes d'arcade]{Des bornes d'arcade, à l'esthétique très connotée années 1980, utilisées comme interface de jeu dans la performance audiovisuelle \textit{Donjon}. A gauche: Vue d'ensemble de la scène, à droite: détail des interfaces de Vincent Goudard. Photographies: Sébastien Bozon.}
	\label{fig:interface:donjon}
\end{figure}
%------------ Figure : Donjon -----------
...
\noindent L'importance croissante du découplage énergétique entre les gestes de l'instrumentiste et la production du son, depuis l'orgue, qui en donne les premiers signes manifestes, jusqu'aux \glspl{DMI}, qui finissent d'achever cette scission, a entrainé une séparation physique de l'interface de jeu. En effet, comme l'énonce Thor Magnusson, \iquote{on peut dire que l'instrument électronique ou numérique \textbf{a} une interface, tandis que l'instrument acoustique \textbf{est} l'interface\footnote{``We can state that the electronic or digital instrument \textbf{has} an interface, whereas the acoustic instrument \textbf{is} the interface.'' \cite{magnusson_sonic_2019}}.}

Cette séparation permet de repenser de manière beaucoup plus ouverte la distribution spatiale de l'instrument, son ergonomie gestuelle, ainsi que la temporalité des relations gestuelle-sonore, dans un champ de possibles qui pourrait s'apparenter à une page blanche. Le design de cette interface est toutefois polarisé par un certain nombre de facteurs matériels (poids, encombrement, matériaux physique, temps de montage, portabilité, etc.) qui conditionnent l'usage effectif des \glspl{DMI} pour la performance musicale.

Mais une fois l'interface séparée, il s'agit donc de recréer sa connection à la production du son.

L'interface de jeu définit la manière dont la part virtuelle et intangible des \glspl{DMI} s'inscrit dans le monde physique en définissant leur affordance. 



Avant même que le musicien ne soit présent pour interagir avec son instrument, la présence physique de l'instrument (ou son absence) se manifeste par son interface sensible. Sa configuration évoque à la fois les postures corporelles et les gestes qu'appelle cette interface, mais possède également une force propre d'évocation poétique.

Les \glspl{DMI} ne sont plus nécessairement des ``objets que l'on touche'' comme les instruments acoustiques (et comme l'évoquent l'italien \textit{toccare}), dans la mesure où les capteurs permettent à la fois d'étendre l'interaction à un espace externe à l'objet qui capte le mouvement (e.g. dans la captation de la danse), ou interne au corps même de l'instrumentiste (en utilisant des bio-capteurs).

Les interviews menées auprès de musiciens et l'étude des pratiques musicales impliquant des \glspl{DMI} indiquent un éventail très large de configurations d'interface, de l'objet physique imposant à sa disparition la plus complète, de sa référence à l'instrument traditionnel (dans les instruments augmentés)
...
Leur diversité les rend ainsi difficiles à classer en raison des multiples héritages dont s'inspire leur design.

L'agencement de ces matériaux hérite de différents domaines: la lutherie acoustique mais également du design industriel des interface de contrôle et de l'ingénierie du son, ainsi que d'une part plus scénographique et poétique.
 

L'interface de jeu peut à la fois être caractérisée par ses aspects fonctionnels, qui permettent de pouvoir effectivement interagir avec la partie informatique, mais également par sa présence physique, qui définit à la fois le rapport du corps et de ses gestes à l'interaction mais également une force d'évocation poétique et métaphorique. 

L'évolution des capacités de calcul des ordinateurs a permi leur intégration progressive dans le corps même de l'instrument.


Les \glspl{DMI}, à la différence des instruments acoustiques, ne sont jamais entièrement construit par une seule et même personne, car les capteurs et processeurs qui les composent sont issus d'une fabrication industrielle qui les rendent dépendants d'une logique de production externe au luthier. Leur fabrication (des DMI) procède ainsi par assemblage de matériaux déjà complexes
...
L'agencement des capteurs sur l'interface de jeu définit la topologie spatiale de l'interaction gestuelle possible. Il est cependant difficile, comme nous l'avons ici objecté, d'établir un lien entre les types de capteurs utilisés et les fonctions musicales que l'on cherche à moduler, tant il est possible d'interpréter et de re-segmenter les données issues des capteurs de différentes manières à l'aide d'algorithmes de \textit{mapping}. 

Le choix de cet agencement semble ainsi inséparable d'une considération globale de l'interaction musicale.
