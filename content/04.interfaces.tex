...
Les \glspl{DMI} ne sont pas purement virtuels: 

L'interface de jeu des \glspl{DMI} définit la manière dont la part virtuelle intangible des DMI s'inscrit dans le monde physique en définissant leur affordance. 

Le design de cette interface est donc polarisé par un certain nombre de facteurs matériels : poids, encombrement, matériaux physique, temps de montage, portabilité qui conditionne leur usage effectif pour la performance musicale.

Avant même que le musicien ne soit présent pour interagir avec son instrument, la présence physique de l'instrument (ou son absence) se manifeste par son interface sensible. Cette présence évoque à la fois les postures corporelles et les gestes qu'appelle cette interface, mais possède également une force propre d'évocation poétique.

Les \glspl{DMI} ne sont plus nécessairement des ``objets que l'on touche'' comme les instruments acoustiques (et comme l'évoquent les termes italiens \textit{toccare, toccata}), dans la mesure où les capteurs permettent à la fois d'étendre l'interaction à un espace externe à l'objet qui capte le mouvement (e.g. dans la captation de la danse), ou interne au corps même de l'instrumentiste (bio-capteurs).

Les interviews menées auprès de musiciens et l'étude des pratiques musicales impliquant des \gls{DMI} indiquent un éventail très large de configurations d'interface, de l'objet physique imposant à sa disparition la plus complète, de sa référence à l'instrument traditionnel (dans les instruments augmentés)
...
L'interface de jeu est à la fois une concrétisation tangible des fonctions instrumentales, mais également 
...
Le choix de cet agencement semble ainsi inséparable d'une considération globale de l'interaction musicale. Cependant 
