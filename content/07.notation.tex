...
\indent La notation possède cependant beaucoup d'autres fonctions. Elle permet notamment de transposer la \textit{temporalité musicale} en une \textit{spatialité visuelle}, permettant au compositeur d'agencer des éléments musicaux ``hors du temps''. La notation musicale (tout comme l'écriture littéraire), a ainsi permis de créer des œuvres qui auraient difficilement pu être conçues sans ce support visuel\footnote{Un exemple notoire est le rondeau ``Ma fin est mon commencement'' (\siecle{14}~siècle) de Machaut, dans lequel les deux voix sont rétrogrades l'une à l'autre.}.\\
