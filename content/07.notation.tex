...
\begin{figure}[!htbp]
	...
	\caption[Partitions de \textit{December 1952} et \textit{Mycène Alpha} (extraits)]{Extraits des partitions de \textit{December 1952} de Earle Brown (à gauche) et \textit{Mycène Alpha} de Iannis Xénakis (à droite)}
...
Après avoir présenté et analysé certains aspects particuliers de la notation musicale qui prennent une importance critique dans la pratique des \glspl{DMI}, j'ai retracé la naissance du logiciel John, un logiciel hybride servant d'outil collaboratif pour la création de parititions minimale, ainsi que de guide temporel durant la performance.
...
\indent À la différence de la plupart des logiciels de composition, John n'a pas été conçue selon la perspective notationelle du compositeur, pour qui la partition occupe généralement une place centrale, mais selon la perspective performative des musiciens dans un contexte improvisé, exigeant une grande attention mutuelle. Son design visuel a été adapté en conséquence pour minimiser la charge cognitive des instrumentistes.

\indent Enfin, la possibilité de venir contôler des processus externes depuis John permet d'envisager d'autres nuances dans l'articulation possible entre œuvre fixe et œuvre improvisée
