...
\begin{figure}[!htbp]
	...
	\begin{minipage}[t]{0.48\textwidth}
		...
		\caption[Brevet sur l'amélioration du clétage des flûtes de Boehm]{Extrait du brevet de D. Julliot sur \iquote{l'amélioration du clétage des flûtes de Boehm}, 1906.}
		\label{fig:visual_representation:boehm}
	\end{minipage}
\end{figure}


\subsubsection{Adaptations ergonomiques}
...
\subsubsection{Représentations liées à la théorie musicale}
...
\subsubsection{Représentations liées à l'expérimentation}
...
\subsubsection{Aspects esthétiques : l'instrument œuvre d'art}
...
\indent En tant qu'élément scénographique, la représentation visuelle joue un rôle essentiel dans l'esthétique et la poésie de l'instrument. Par exemple, dans la performance \textit{FIB\_R}\index[people]{forages@FORAGES (groupe)!fibr@\textit{FIB\_R}} : toute la performance visuelle vidéo-projetée sur l'écran visible du public correspond à ce que les instrumentistes ont face à eux sur leur tablette (voir figures \ref{fig:interface:filigramophone-Xypre-Triton} et \ref{fig:visual_representation:xypre-live}). Le design de l'interface graphique est donc autant motivé par des aspects fonctionnels d'interaction, que des aspects esthétiques et scénographiques.
...
\noindent En tant qu'\textit{outils épistémiques}, les \glspl{DMI} mettent en jeu un nombre élevé de flux de données et de processus qui interagissent entre eux et avec l'interface. La représentation visuelle peut faciliter le \textit{monitoring} du bon fonctionnement de ces processus, de leurs dynamiques, et la sélection de réglages dans des banques de données (presets, samples, etc.). Ce monitoring s'avère utile, voir nécessaire, tant durant l'élaboration de l'instrument que lors de sa pratique, même si les besoins ne coïncident pas nécessairement lors de ces différentes phases.
...
\indent À mesure que les données emmagasinables sur les ordinateurs augmentent --~et plus encore, sur le net en tant qu'il constitue une excroissance de l'instrument numérique~-- la fonction de cartographie et de recherche de l'information utile prend de plus en plus d'importance. On peut en observer les signes dans la généralisation des moteurs de recherche dans les logiciels, notamment audio. Le numérique fait évoluer notre rapport aux outils d'une posture consistant à ``connaître leur fonctionnement'', à une posture consistant à ``savoir où chercher les fonctionnements possibles''. Nicolas Collins remarque à ce sujet, non sans une certaine ironie:
\begin{quotation}
``(...) comme me le disent mes étudiants, le plus gros problème qu'ils rencontrent dans leur production musicale est de retrouver leurs échantillons audio sur leurs disques durs. Ils me disent : "J'ai tellement de kicks de batterie, que j'ai téléchargés de tellement de sites différents, que je n'arrive jamais à trouver celle que je veux". (...) Cela signifie peut-être que nous n'avons pas besoin de Stradivarius, mais d'un programmeur de base de données; que la chose la plus importante dans votre vie serait une base de données vraiment intelligente, qui vous permettrait de retrouver intuitivement n'importe quel échantillon que vous voulez... En d'autres termes, il s'agit d'un domaine très différent, mais c'est peut-être, comme je l'ai dit, la chose la plus importante pour un compositeur travaillant aujourd'hui.'' Nicolas Collins, entretien personel, ma traduction.
\end{quotation}

\indent Cependant, l'action de rechercher un contenu n'est pas \textit{a priori} un geste expressif. La conception d'un \gls{DMI} pour la performance passe ainsi par la circonscription d'un territoire dans lequel l'interaction soit suffisamment directe et intuitive pour que l'instrumentiste n'ait pas à réfléchir à la manière de se rendre à l'endroit désiré\footnote{Comme le note encore Collins:\iquote{(...) it's a difficult thing to articulate but I think that if you give somebody too many choices of things to do, it will not be an expressive musical instrument because there's gonna be too much thinking (...) you have to sort of find a spot in between}, Nicolas Collins, entretien personel.}.

\subsubsection{Temporalité de l'interface} 

\noindent À la différence d'une interface physique, l'information sur écran n'est pas seulement représentée de manière \textit{spatiale}, mais également \textit{temporelle}, ce qui offre des possibilités tout à fait adaptées pour la représentation des phénomènes sonores et des processus musicaux. Ainsi, l'écran peut assurer une représentation du tempo, des patterns rythmiques ou encore de la progression d'un processus (e.g. l'état d'avancement d'un séquenceur) dans leur évolution dynamique, ce qui facilite leur anticipation (voir la figure \ref{fig:visual_representation:dialButton}).\\
%------------ Figure : skeuomorphisme et soin du design -----------
\begin{figure}[!htbp]
	\captionsetup{format=plain}%
	\centering
...
\begin{itemize}[noitemsep]
	...
	\item la cohésion entre le contrôle d'un objet et sa représentation, et la relation sensuelle d'intimité qui se créé en l'absence de distance entre l'objet virtuel et celui ou celle qui le manipule.
\end{itemize}
%-------------
\noindent La cohésion entre la représentation et le contrôle d'un objet virtuel\footnote{... qui, rappelons-le, n'est qu'une ``synchrèse'', c'est-à-dire un artéfact liant les deux phénomènes par leur concomitance spatio-temporelle.} facilite l'intuitivité de l'interface en abolissant la distance entre ces deux aspects de l'interaction. Au-delà de cet intérêt ergonomique, cela permet d'imaginer de nouveaux scénarios d'interactions dynamiques. Même de simples interactions usuelles, comme celle de la bureautique (sliders, boutons, etc), peuvent être ré-inventées avec des comportements alternatifs, dans une perspective d'utilisation musicale, comme nous le verrons plus loin. Cette idée est notamment poursuivie dans le domaine des \gls{IHM}, avec le dessein d'intégration du numérique dans les objets du monde physique, telle que présenté notamment dans l'article ``Tangible Bits'' d'Ishii et Ullmer \cite{ishii_tangible_1997} et reprise par Sergi Jordà dans les 25 principes qu'il adopte pour le développement de la ReacTable\footnote{``For including realtime interactive visualizations and, at the same time, overcoming mouse limitations without adding indirections, interfaces should be able to reflect their own states and behaviors. They should integrate, like the abacus, both representation and control.'' \cite{jorda_digital_2005}}. Ce faisant, la cohésion entre le geste et la représentation ``acquise'' dans les écrans tactile laisse la place à des scénarios d'interaction subversifs, qui se jouent de cette synchrèse artificielle\footnote{Voir en particulier la série de performances ``Screen to screen'' de l'artiste Vincent Brocquaire\index[people]{brocquaire@Brocquaire, Vincent!screentoscreen@\textit{Screen to screen}}, basées sur cette duplicité de l'écran/interface. \url{http://www.vincentbroquaire.com/screen-to-screen.html}. Sur le geste subversif, cf. section \ref{sec:gesture:subversion}.}.\\
...
\begin{itemize}[noitemsep]
	...
	\item \textbf{amovible} : les repères peuvent être changés ``physiquement'', soit que leurs positions soient réglables (comme c'est le cas pour les frettes du sitar), soit qu'ils soient interchangeables (e.g. les \textit{overlays} des interfaces \textit{Joué} et \textit{Sensel Morph}, ou le calque glissable dans une tablette graphique);
...
\end{itemize}
\noindent On retrouve des règles similaires dans les (nombreuses) préconisations concernant le design d'interfaces graphiques, guidées par une recherche de fonctionnalité, lisibilité et transparence maximales\footnote{Ce qui correspond à la tendance dominante du ``Design plat'' adopté par les grandes industries depuis la dernière décennie: le ``Material Design'' de Google, le ``Modern UI'' de Microsoft, et celui d'Apple depuis iOS 7.}. Bien que toutes ces règles puissent être utiles pour le design de la partie graphique d'un \gls{DMI}, il faut cependant garder en tête que les instruments de musique ne sont pas des interfaces dont la raison d'être est définie par leur fonctionnalité (on n'achète pas un instrument pour se rendre la vie plus facile), et que les notions d'\textit{efficacité}, de \textit{lisibilité} et de \textit{vérité} y sont particulièrement sujettes à caution, comme nous l'avons déjà dit.
...
\begin{figure}[!htbp]
	\includegraphics[width=\textwidth]{gfx/06_visual_representation/mp-TUI-preview.png}
...
\begin{figure}[!htbp]
	...
	\begin{minipage}[t]{0.36\textwidth}
		...
		\caption[Composants MP.TUI groupés]{Composants groupés. Le cadre du groupe est représenté en rouge.}
