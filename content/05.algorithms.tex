...
\begin{itemize}[noitemsep]
	...
	\item \textbf{les flux}, provenant des capteurs de l'interface, du réseau, ou du code lui même, dans sa capacité à les générer, et qui se modifient en ``traversant'' le code pour produire \textit{in fine} des signaux audio.
\end{itemize}
...
\vspace{-1em}
\begin{itemize}[noitemsep]
	\item \textbf{des signaux synchrones}, représentant par exemple des signaux audio, échantillonnés à une fréquence précise et traités généralement par blocs dans un \gls{DSP};
...
\indent Ce terme a glissé dans le vocabulaire de la lutherie numérique en raison de la relative simplicité des relations de correspondance dans les premiers \glspl{DMI} et est resté, alors que ces relations se sont progressivement complexifiées, comme le notait déjà Joel Chadabe en 2002:
\begin{quotation}
\iquote{Le mapping décrit la façon dont un contrôle est connecté à une variable. Mais à mesure que les instruments deviennent de plus en plus complexes pour inclure de grandes quantités de données, une sensibilité au contexte, ainsi que des capacités de génération sonore et musicale, le concept de mapping devient plus abstrait et ne décrit pas les réalités plus complexes des instruments électroniques.}\footnote{\iquote{Mapping describes the way a control is connected to a variable. But as instruments become more complex to include large amounts of data, context sensitivity, and music as well as sound-generating capabilities, the concept of mapping becomes more abstract and does not describe the more complex realities of electronic instruments.} \cite{chadabe_limitations_2002}}
\end{quotation}

\subsubsection{Un grand répertoire}

\noindent Le \textit{mapping} est un domaine en effervescence depuis le début des années 2000 et de nombreux modèles ont été proposés (figures \ref{fig:algorithms:DynamicMappingLayer1}, \ref{fig:algorithms:DynamicMappingLayer2}, \ref{fig:algorithms:DynamicMappingLayer3}), passant par la proposition de modèles multi-couches \cite{wanderley_escher_1998, hunt_mapping_2002}, auto-adaptatifs \cite{verfaille_mapping_2006}, de cartographie de paramètres sur des espaces perceptifs \cite{wessel_timbre_1979, wyse_instrumentalizing_2010,schwarz_sound_2012, tubb_divergent_2014}, de métaphores d'interaction avec des objets virtuels \cite{wessel_intimate_2002}, de modèles physiques ou visuels \cite{momeni_dynamic_2006}, de modèles géométriques \cite{van_nort_choice_2004}, stochastiques \cite{dabby_musical_1996}, ou d'apprentissage automatique \cite{fiebrink_real-time_2011, caramiaux_mapping_2014, francoise_motion-sound_2015}.\\
...
\begin{figure}[!htbp]
	...
	\caption[Aperçu très partiel d'un patch Max]{Aperçu très partiel du patch Max utilisé dans la performance ``FIB\_R'' de Forages\index[people]{forages@FORAGES (groupe)!fibr@\textit{FIB\_R}}.}
...
\indent Parmi les qualités recherchées lors de l'utilisation de modèles intermédiaires, se trouvent en bonne place les qualités que l'on trouve sur les instruments acoustiques, perdues dans leur transposition numérique, à savoir, la finesse et la richesse de l'interaction avec des matériaux: frottement, grincement, raclement... et tous les phénomènes subtils et non-linéaires qui se produisent en fonction de l'état de surface, de la viscosité et des propriétés élastico-mécaniques des matériaux. Certains de ces modèles s'inspirent ainsi de systèmes intermédiaires rencontrés dans le domaine des instruments acoustiques (si l'on considère, par exemple, l'archet comme un système intermédiaire entre le geste du violoniste et la corde). D'autres sont plus spécifiques au numérique, tels que les modèles agissant sur l'aspect temporel du geste ou sur sa reconnaissance.\\
\indent La liste suivante présente plusieurs axes selon lesquels ces modèles affectent le mouvement :
\vspace{-1em}
\begin{itemize}[noitemsep]
	\item \textbf{en qualité}: les modèles intermédiaires peuvent venir enrichir le mouvement, que cela soit de manière statique, en modifiant sa plage de valeur, ou dynamique, en le modulant pour lui ajouter un contenu fréquentiel absent dans le geste capté, excédant potentiellement les capacités motrices humaines\footnote{Nick Collins présente ainsi son algorithme BBcut (pour \iquote{Break Beat Cutter}), qui opère un découpage et un ré-ordonnancement d'un échantillon audio (typiquement une boucle de batterie) dans le style frénétique de la drum'n bass: ``\textit{The sort of audio cutting assumed here is usually at haptic or human rhythmic rates, with the obvious capability to reach inhuman speeds.}'' dans \cite{collins_bbcut_2002}. }, par exemple, un vibrato, un tremolo, une distorsion, etc. Le champ des possibilités est très vaste, tant la richesse du signal fourni par les capteurs gestuels sur les \glspl{DMI} est généralement bien en-deçà de la richesse des interactions se produisant dans les instruments acoustiques.\\
	\indent Il est aussi possible qu'un modèle intermédiaire \textit{réduise} la richesse du mouvement, pour borner son amplitude ou réduire ses variations. Un exemple notoire dans les musiques actuelles est l'\textit{auto-tune} qui vient réduire les variations de hauteur dans la voix naturelle, pour la contraindre sur une hauteur fixe. Dans le cas du geste, le système de quantification dynamique de la hauteur présenté dans \cite{goudard_playing_2014} en est un exemple (cf. figure \ref{fig:algorithms:MID-dynamic-pitch-correction}).\\
	\indent La nature de l'altération vient conférer une identité musicale au modèle intermédiaire, un ``son'' spécifique. Par exemple, l'algorithme BBcut \cite{collins_bbcut_2002} et ses variations\footnote{Notamment : \textit{modsquad} (2000) de Atau Tanaka\index[people]{tanaka@Tanaka, Atau} dans Max, \textit{LiveCut} de Rémy Muller, ou encore \textit{BeatRepeat} (2005) dans Ableton Live.} est une signature qui, associée à une famille de boucles rythmiques\footnote{Notamment les samples ``\textit{Amen Break}'' des Winstons, ``\textit{Funky Drummer}'' de James Brown, ou encore ``\textit{Think}'' de Lyn Collins.}, est audible dans toute la musique breakbeat et ``incorpore'' --~pour reprendre l'expression de Manovitch en exergue de ce chapitre~-- les techniques de scratch des DJs et de segmentation de boucles rythmiques re-séquencées dans les boites à rythmes.

	\item \textbf{en quantité}: le geste peut également être augmenté quantitativement, en le démultipliant et en créant une polyphonie à partir d'un mouvement unique. C'est le cas, par exemple, de l'arpégiateur qui génère un accord arpège à partir d'un accord plaqué ou simplement de sa note fondamentale, ou encore des algorithmes qui permettent de générer une multitude de mouvements comme la réponse d'un objet virtuel sur lequel on exerce une simple force\footnote{Voir par exemple le projet ``Silicat'' de Mathieu Constans, qui manipule en direct des personnages virtuels (et sonores) tel un marionnetiste: \url{https://vimeo.com/362433354}, ou le projet ``Espaces croisés'' de Mathieu Chamagne, qui manipule en temps réel des modèles physiques: \url{https://youtu.be/InY8bwlVOsg}}. Le mouvement peut également être réduit en quantité, notamment par des systèmes de cribles qui ne laissent passer que certaines valeurs.
	...
	\item \textbf{dans sa temporalité}: les machines permettant l'enregistrement et le temps différé offrent de multiples possibilités d'étirer, de contracter, de différer le mouvement, que cela soit sous la forme d'échos, de boucles enregistrées, d'alignements temporels sur une partition\footnote{Voir par exemple le système \textit{Antescofo} qui permet de suivre une partition en temps-réel, et de déclencher des événements pré-enregistrés, en s'adaptant à la vitesse de l'instrumentiste.} ou sur un motif rythmique, comme les ``\textit{grooves}'' dans Ableton Live qui permettent d'introduire différentes sortes de \textit{swing} sur des motifs rythmiques. %De même, les algorithmes de reconnaissance de formes permettent de traduire la continuité d'un geste en un événement discret, comme c'est le cas dans les 

	\item \textbf{dans son contenu}: un geste peut également être le simple déclenchement de processus possédant leurs propres mouvements autonomes, comme cela peut être le cas dans les algorithmes stochastiques ou génératifs (cf. modèle basé sur le ``jeu de la vie'', présenté dans les exemples en section \ref{sec:algorithms:MID:examples}). Un autre exemple intéressant est celui du sélecteur de bande radio, qui transforme un simple geste de rotation de potentiomètre en des changements disruptifs de contenus musicaux, tel qu'utilisé par John Cage dans sa pièce ``Imaginary Landscape No. 4''\index[people]{cage@Cage, John!imaginarylandscapenofour@\textit{Imaginary landscape No 4}}, et qui a été adapté dans un module de synthèse modulaire Eurorack.
\end{itemize}

\noindent Indépendamment de leur fonctionnement interne, les modèles intermédiaires présentent également certaines potentialités, qui facilitent leur usage au sein d'un agencement instrumental. La liste ci-après résume quelques aspects souhaitables de cette affordance :
\vspace{-1em}
\begin{itemize}[noitemsep]
	\item \textbf{disponibilité} : pouvoir les convoquer et les délaisser à tout instant, en cours de geste;
	\item \textbf{interconnectibilité} : pouvoir facilement brancher un modèle sur un autre, sans que la nature ou la valeur de leurs signaux ne restreigne \textit{a priori} cette connexion;%\todo{add footnote with ref to DIM video ?};
	...
	\item \textbf{abusabilité} : ne pas contraindre arbitrairement les plages de valeurs selon des critères de raisonnabilité \textit{a priori}, afin de laisser l'espace nécessaire à une expérimentation ``hors-normes'' et une relation subversive, telle que nous l'avons précédemment décrite. Par exemple, il est intéressant de pouvoir utiliser un modèle physique dans un régime incohérent avec la physique réelle.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \subsection{Exemples}
% \label{sec:algorithms:MID:examples}
% \todo{Virer toute cette partie}
% %------------------ Figure : MID ---------------------
% \begin{figure}[!htbp]
% 	\captionsetup{format=plain}
...
\noindent Nous avons vu dans la section précédente comment le concept de modèle intermédiaire dynamique pouvait enrichir le geste capté et améliorer l’ergonomie des \glspl{DMI}. Cependant, un des facteurs critiques rencontré lors de ces développements se situait dans la manière de faire communiquer différents modules polyphoniques entre eux. En effet, la polyphonie dans les \glspl{DMI} peut être envisagée à plusieurs niveaux: polyphonie de gestes (par exemple, plusieurs doigts en contact sur une tablette multitouch), de sons (par exemple, plusieurs ``notes'' qui résonnent en même temps) ou de modèles intermédiaires (par exemple, plusieurs processus qui tournent en parallèle). Concevoir le design d'un \gls{DMI} poly-poly-polyphonique nécessite par conséquence d'établir les connexions entre ces différentes multiplicités.\\
...
%\todo{Add a note on why this matters + intégrer cette partie dans la sous-section suivante}
...
Les limitations du \gls{MIDI} ont pourtant été identifiées peu de temps après son apparition\footnote{Voir notamment les critiques dans \cite{moore_dysfunctions_1988}, \cite{mcmillen_zipi_1994} ou \cite{selfridge-field_beyond_1997}.}, notamment :
\vspace{-1em}
\begin{itemize}[noitemsep]
...
\noindent Dans cette brève revue, il faut mentionner \gls{TUIO} \cite{kaltenbrunner_tuio:_2005}, basé sur \gls{OSC}, et développé pour le projet \textit{ReacTable}. \gls{TUIO} ne se focalise que sur l'information gestuelle particulière propre aux interfaces multitouch, en transmettant des informations sur le contact des doigts et d'éventuels objets sur la surface tactile. Il introduit un indice incrémentiel pour identifier de manière unique des événements dynamiques et éphémères tels que les touchers de doigt sur une \gls{TUI}). Il se distingue également de la logique des événements \gls{MIDI}, dont l'utilisation de messages distincts pour les \textit{note-on} et \textit{-off} peut produire des notes qui restent ``bloquées'' si un message \textit{note-off} est perdu. Il propose une solution simple et pratique pour résoudre l'incertitude d'arrivée des messages envoyés sur \gls{UDP}, consistant à envoyer systématiquement la liste des événements actifs.

\subsubsection{Minuit/Jamoma}

\noindent Une autre direction de développement basée sur \gls{OSC} est le protocole  \textit{Minuit} \cite{de_la_hogue_jamoma_2011}, qui implémente en particulier un système d'auto-déclaration / découverte sur un réseau de modules connectés. \textit{Minuit} a été utilisé comme protocole dans le projet Jamoma\footnote{\url{http://jamoma.org}} et dans le séquenceur interactif i-score/OSSIA développé par le \gls{LaBRI} \cite{celerier_ossia:_2015}.

\subsubsection{Le signal}

\noindent S'il n'est généralement pas envisagé tant comme un signal de contrôle que comme un support pour l'audio numérique\footnote{David Zicarelli, PDG de Cycling'74, parle encore d'\textit{audio channels} dans une conférence présentant la nouvelle version de \gls{Max} pourtant censée changer la manière de les considérer : \url{https://youtu.be/Y4YLy7kqcr8}}, le signal peut être utilisé à cette fin et même permettre la transmission d'événements asynchrones. Le développement de la librairie ``Sagrada'', présentée dans la section \ref{sec:algorithms:sagrada}, en donne un exemple concret.

\subsubsection{MIDI 2.0}
\noindent Le protocole \gls{MIDI} 2.0 prend en charge un grand nombre de lacunes du \gls{MIDI} 1.0. Étant sorti juste après la rédaction de la thèse dont est issu cet ouvrage, il se trouve qu'il répond à un certain nombre de problèmes pris en charge par le protocole MP proposé ci-après, notamment la possibilité d'adresser et moduler chaque note séparément, la séparation entre l'identifiant de note et son \textit{pitch}, la séparation entre l'extinction d'une note (\textit{note-off}) et une vélocité nulle et un système d'auto-déclaration / découverte. Sa description détaillée\footnote{Consulter \url{https://www.midi.org/specifications/midi-2-0-specifications}.} dépasse le cadre de ce qui nous intéresse ici. Notons toutefois que cette mise à jour du protocole \gls{MIDI} reste cantonnée aux notions générales définies dans le \gls{MIDI} 1.0, pour des raisons compréhensibles de cohérence et de rétro-compatibilité, quand le protocole défini ci-après est plus ouvert.
...
Notons toutefois que dans notre cas, le message d'état peut être rattaché à n'importe quel paramètre, ce qui diffère encore de l'implémentation \gls{MIDI}, notamment en ce qui concerne l'allocation de voix. Alors que le premier message \verb|state 1| reçu pour un ID causera l'allocation d'une voix dans le \textit{MP-block}, le message \verb|state 0| ne libérera pas nécessairement cette voix, cette décision revenant au processus en cours, comme nous le verrons plus loin.
...
\begin{figure}[!htbp]
	...
	\begin{minipage}[t]{0.485\textwidth}
		...
		\caption[MP : mapping avec une fonction pure]{Exemple de patch MP : mapping avec une fonction pure.}
		\label{fig:algorithms:MP-pure}
	\end{minipage}
	\hspace{.01\linewidth}
	\begin{minipage}[t]{0.485\textwidth}
	  	\includegraphics[width=\linewidth]{gfx/04_algorithms/MP-mappingRecursive.png}
		\caption[MP : mapping avec une fonction récursive]{Exemple de patch MP : mapping avec une fonction récursive.}
...
\begin{figure}[!htbp]
	...
	\begin{minipage}[t]{0.38\textwidth}
		...
		\caption[MP : guestlist et mapping divergent]{Utilisation de la guestlist pour un mapping divergent.}
		\label{fig:algorithms:MP-mappingDivergent}
	\end{minipage}
	\hspace{.01\linewidth}
	\begin{minipage}[t]{0.58\textwidth}
	  	\includegraphics[width=\linewidth]{gfx/04_algorithms/MP-mappingConvergent.png}
		\caption[MP : guestlist et mapping convergent]{Utilisation de la guestlist pour un mapping convergent.}
...
\begin{figure}[!htbp]
	...
	\caption[Sagrada : schéma fonctionnel du \textit{jump-buffer}]{Schéma fonctionnel de la computation synchrone du \textit{jump-buffer}}
