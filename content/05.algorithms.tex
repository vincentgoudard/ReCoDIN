...
\noindent Dans le cas des \glspl{DMI}, les données sont essentiellement de qutre types:
\vspace{-1em}
\begin{itemize}[noitemsep]
	\item \textbf{des flux de données}, qu'ils soient audio ou non, captés en temps-réel ou re-lus à partir d'enregistrements;
	\item \textbf{des algorithmes}, définissant les opérations mathématisques à effectuer pour traiter les données audio
	\item \textbf{des objets}, définissant des structures de données: réglages et xxx todo;
...
\begin{figure}[!htbp]
	...
	\begin{minipage}[t]{0.48\textwidth}
	  \includegraphics[width=\linewidth]{gfx/04_algorithms/mapping-IRL.pdf}
		\caption[Représentation du mapping \#4]{Mapping, ``dans la vraie vie'' (schéma simplifié)}
...
\noindent La polyphonie dans les \gls{DMI} peut être envisagée à plusieurs niveaux: polyphonie de gestes (e.g. plusieurs doigts en contact sur une tablette multitouch), de sons (e.g. plusieurs ``notes'' qui résonnent en même temps) ou de modèles intermédiaires (e.g. plusieurs processus qui tournent en parallèle). Concevoir le design d'un \gls{DMI} poly-poly-polyphonique nécessite par conséquence d'établir les connexions entre ces différentes multiplicités.
