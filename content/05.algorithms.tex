...
\paragraph{keywords}{ergodynamisme, résonance, mapping.\\En général, découplage conceptuel (et logiciel) entre la partie DSP et la partie "contrôle". Dans la réalité, ce n'est pas si simple et c'est la raison pour laquelle ce qui est habituellement rangé du côté "mapping" et du côté "synthèse" est rassemblé dans un seul et même chapitre.} 

Si la partie "hard" d'un instrument (sa prise en main, son poids, sa forme, la disposition de ses cordes, etc.) est intimement couplée à sa résonance dans la plupart des instruments acoustiques, cette relation n'est pas de mise dans le cas d'un DMI. Une sorte d'équivalent à la résonnance de l'instrument 
...
On a tendance à séparer, dans la description et le design des DMI, ce qui est du côté de la synthèse, souvent associé voire confondue à son support de \textit{signal}, de ce qui est du côté du contrôle, souvent associé à des données de type \textit{messages asyncrones} (e.g. typiquement, des messages MIDI).\\
Il existe une différence de nature assez profonde entre ces deux types de données, l'une étant un signal synchrone, et se proposant souvent comme l'i\textit{mage d'un signal analogique échantillonné} avec l'idée de continuité qui lui est associée.\\
