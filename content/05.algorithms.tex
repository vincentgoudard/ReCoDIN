...
De toute évidence, parmi les qualités recherchées lors de l'utilisation de MID, se trouvent en bonne place les qualités que l'on trouve sur les instruments acoustiques, perdue dans leur transposition numérique : finesse et richesse de l'interaction avec des matériau (le frottement, le grincement, le raclement, et tous les phénomènes subtils qui se produisent en fonction de l'état de surface, de leur viscosité et des propriétés mécaniques des matériaux)

\noindent Le rôle des MIDs est de transformer le mouvement de diverses manières. Certains de ces modèles s'inspirent d'équivalents dans le domaine des instruments acoustiques, qui usent eux aussi de systèmes --~plutôt que de modèles~-- intermédiaires (si l'on considère, par exemple, l'archet comme un système intermédiaire entre le geste du violoniste et la corde). D'autres sont plus spécifiques au numérique. La liste suivante présente plusieurs axes selon lesquels ces modèles affectent le mouvement :
...
- créer la discontinuité (fretting, gamme, seuils de déclenchement)

- créer la continuité (portamento, glissando, enveloppes temporelle, filtrage, lissage, interpolation)

- modifier la qualité (modulation (e.g. vibrato), modèles physiques résonnant, propriétés) réduire la qualité (vocodage)

- modifier la quantité (créer une polyphonie, contrôler une foule d'agents autonomes) (filtrage, garde-fou, quantification, alignement)

- permettre la navigation : cette dimension cartographique est liée à la quantitée très élevée de données présentes et potentiellement convocables sur un ordinateur

- accommpagner le mouvement : suivi de partition, extrapolation du geste (résonandce)

- changer leur temporalité : dilater, compresser, différer

- analyse et reconnaissance <= créer la discontinuité ?

- créer la surprise (modèles aléatoires, génératifs (voronoi), stochastiques (cf. F. Dumeaux radio))


affordance :

- disponibilité : pouvoir les convoquer et les délaisser à tout instant (i.e. en cours de geste)
- interconnectibilité : pouvoir les interconnecter
- ajustabilité : 
- éditabilité : pouvoir les éditer et créer des variations
- réutilisabilité : pouvoir les ré-utiliser dans d'autres contextes
- abusabilité : ne pas contraindre arbitrairement les plages de valeurs selon des critères de raisonnabilité a priori
%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Affordance des modèles intermédiaires}
