...
\item \textbf{la performance} : le jeu instrumental qui produit, modifie, mélange, dans le temps même de l'écoute, la matière sonore, les gestes de l'instrumentiste, du ou de la chef d'orchestre, de l'ingénieur du son;
...
%\footnote{L'expression utilisée ici renvoie bien évidemment à l'œuvre de Walter Benjamin \cite{benjamin_loeuvre_2013} qui analyse, pour sa part, les conséquences de cette révolution technique sur la réception des œuvres d'art, selon une perspective socio-politique. Il faut y ajouter aujourd'hui la reproduction technique à l'ère l'\gls{IA}}\todo{ref Benjamin bof}
, qui sera analysée à la lumière de la notion de (dia)gramme proposée par Gilles Deleuze et Bernard Stiegler.
...
%Dans son ouvrage ``\textit{Playing with something that runs}''\cite{butler_playing_2014}, Mark J. Butler propose une analyse des différents modes d'interaction lors d'improvisation sur des outils technologiques ``qui avancent'' (tels que les platines vinyle des \glspl{DJ} ou les laptops des musicien·ne·s numériques). Il distingue ainsi sept types d'interaction: \iquote{repeating, cycling, going, grooving, riding, transitioning, and flowing}\footnote{Je ne me risquerai pas à traduire ces termes directement, tant les connotations associées à chacun d'eux en anglais diffèrent de celle du français --~ par exemple, ``cycling'' évoque l'idée de répéter de manière cyclique mais également de rouler à vélo. Notons également que Butler s'intéresse ici principalement à l'\gls{EDM}.}, dans lesquels on retrouve cette problématique propre aux instruments électroniques
...
\index[people]{gould@Gould, Glenn!variationgoldberg@\textit{Variations Goldberg}}
	...
	\begin{flushright}
		\begin{minipage}[t]{0.93\linewidth}
			...
			\label{fig:gesture:Bach_partita}
		\end{minipage}
	\end{flushright}
	\end{figure}
	%------------------ Figure : Partita ---------------------
...
\indent La réalisation de cette subversion passe par une \textit{soudure} entre deux espaces: celui physique des gestes et des sons de l'instrumentiste, et celui, psychique, de leur perception et de l'imaginaire qu'ils stimulent, que l'on vient faire coïncider pour extrapoler les mouvements de l'un vers l'autre. L'invisibilité (et l'inaudibilité) de cette soudure, nécessaire à l'impression de continuité, implique qu'elle se glisse dans les synchronismes spatiaux et/ou temporels du jeu instrumental: synchronisation des mains, des pieds, ou du souffle sur les instruments acoustiques, à laquelle s'ajoute la synchronisation aux ``gestes programmés'' sur les \glspl{DMI}, qui ont leur mouvement propre.\\ 
