...
\subsubsection{La musique et ses instruments}
...
\indent \textbf{aspect phénoménologique} Un premier aspect du geste est sa nature en mouvement, son inscription spatiale et temporelle. En soi, le geste possède des qualités de mouvements, en dehors de toute interprétation. Ce mouvement est l'expression même du vivant et il reflète à la fois la relation du corps à son environnement externe (force de gravité et cinétique, obstacles...) et le ``débordement'' des mouvements internes du corps (vigueur ou mollesse, souple ou crispé, émotion se traduisant par de gestes : haussement d'épaule, etc.). La maitrise de ces mouvements qu'on appelle souvent ``purs'' est un aspect fondamental de la danse. Si les gestes de la danse ne sont pas \textit{a priori} instrumentaux, les \glspl{DMI} rendent plus que jamais possible leur utilisation à des fins d'interaction musicale (cf. notamment \cite{bevilacqua_gesture_2011, alaoui_movement_2012, silang_maranan_designing_2014, hsueh_understanding_2019}.)

\indent \textbf{aspect sémiotique} Un deuxième aspect du geste est indiqué par le curieux épithète de la définition précédente : ``porteur de sens ou non''. S'il a semblé utile d'évoquer une qualité potentiellement absente, c'est précisément parce les différentes définitions qu'on donne du geste dépendent en partie de cet aspect sémiotique, qui reste soumis à une interprétation subjective et contextuelle dans un système de valeurs. Les significations d'un geste peuvent en effet être attribuées par l'auteur du geste ou bien par celui qui l'observe, sans qu'il n'y ait nécessairement de correspondance, ni en terme de signification, ni sur la part du mouvement considérée signifiante. La signification d'un geste peut également être attribuée par la machine, en particulier dans les systèmes d'apprentissage.
le geste qui émane d'une intention signifiante et le(s) signification(s) qui émerge(nt) d'un geste

Nous évoquerons brièvement cet aspect sémiotique du geste.

\indent \textbf{aspect ergotique} Une autre définition du geste relève sa dimension manipulative : \iquote{Manière de mouvoir le corps, les membres et, en particulier, manière de mouvoir les mains dans un but de préhension, de manipulation}. Cette définition relève la situation de relation entre le geste et un objet, un outil, un instrument et s'applique par conséquence particulièrement bien à la situation instrumentale. C'est sur cette aspect que se concentre notamment les typologies gestuelles proposées par Claude Cadoz que nous discuterons plus loin.

\indent \textbf{aspect (dia)grammatique)} Enfin, si le geste \textit{ex-prime} (du latin \textit{ex-premo} : ``faire sortir en pressant'') des mouvements internes du corps, qu'ils soient d'origine émotionnelle, sémiotique ou simplement dynamique, le geste \textit{im-prime} et laisse une trace dans la matière, dans la machine, dans les esprits. C'est parfois cette trace résultante qu'on nomme geste, par métonymie et analogie avec les qualités de mouvement qui sont à son origine, lorsqu'on parle par exemple du geste de composition ou du geste de la peinture. Les capacités des machines à enregistrer le geste dans sa dynamique --~auparavant éphémère et seulement visible dans sa trace~-- confèrent à ce sens particulier du geste une importance qu'il nous faut souligner et qui sera analysée en dernier lieu.
...
Claude Cadoz fait partie des pionniers dans l'analyse du geste instrumental, en prenant en compte les spécificités propres à cette pratique gestuelle. En particulier, comme son nom l'indique, le geste instrumental n'est pas un ``geste nu'' mais se retrouve couplé à un instrument qui polarise (sans toutefois les définir totalement) les termes de leur interaction. Le geste est instrumentalisé, médiatisé, et sa perception est prise (de manière indissociable ?) à la perception globale de l'instrument et de la réalité (sonore) qu'il engendre.
[Présenter ici la typologie de Cadoz]
Toutefoi, si cette catégorisation explique en partie le rôle fonctionnel du geste dans l'interaction instrumentale acoustique, les fondements sur lesquels s'appuie Cadoz semble mal convenir pour décrire les gestes instrumentaux dans le cas de l'usage de DMI :
- continuum gestuel
- stabilité de l'instrument
- contact avec l'instrument
=> Plutôt que de constater leur absence dans le cas des DMIs, Cadoz s'en servira de moteur pour le développement des recherches théoriques et pratiques à l'ACROE : modèle physique, retour de force.

Le problème de cette classification est qu'elle laisse entendre un rapport pré-déterminé entre le geste et le son, basé sur le modèle acoustique, i.e. un geste d'excitation (e.g. geste percussif) se traduira par une excitation dans le résultat audible (i.e. un transitoire d'attaque et une énergie communiquée).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Le modèle instrumental acoustique}
...
% \iquote{Nous appelons ici gestes de l’instrumentiste les actions physiques effectuées par le musicien en situation de jeu instrumentale.} \cite{wanderley_controgestuel_1999}
